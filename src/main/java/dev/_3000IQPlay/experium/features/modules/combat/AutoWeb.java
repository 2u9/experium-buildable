...
extends Module {
    ...
    private EntityPlayer getTarget(double range, boolean trapped) {
        ...
        for (EntityPlayer player : AutoWeb.mc.world.playerEntities) {
            if (EntityUtil.isntValid((Entity)player, range) || trapped && player.isInWeb || EntityUtil.getRoundedBlockPos((Entity)AutoWeb.mc.player).equals((Object)EntityUtil.getRoundedBlockPos((Entity)player)) && this.antiSelf.getValue().booleanValue() || Experium.speedManager.getPlayerSpeed(player) > this.speed.getValue()) continue;
            if (target == null) {
                target = player;
