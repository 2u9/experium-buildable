...
extends Module {
    ...
    private boolean noNearbyPlayers() {
        return this.mode == Mode.CRYSTALS && Offhand.mc.world.playerEntities.stream().noneMatch(e -> e != Offhand.mc.player && !Experium.friendManager.isFriend((EntityPlayer)e) && Offhand.mc.player.getDistance((Entity)e) <= this.cTargetDistance.getValue().floatValue());
    }
    ...
    public void setMode(Mode2 mode2) {
        this.currentMode = this.currentMode == mode2 ? Mode2.TOTEMS : (this.cycle.getValue() == false && this.crystalToTotem.getValue() != false && (this.currentMode == Mode2.CRYSTALS || this.currentMode == Mode2.OBSIDIAN || this.currentMode == Mode2.WEBS) && mode2 == Mode2.GAPPLES ? Mode2.TOTEMS : mode2);
    }
