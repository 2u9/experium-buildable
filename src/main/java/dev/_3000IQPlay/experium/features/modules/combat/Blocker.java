...
extends Module {
    private Setting<Float> range = this.register(new Setting<Float>("Range", Float.valueOf(6.1f), Float.valueOf(1.0f), Float.valueOf(10.0f)));
    ...
    public void onTick() {
        ...
        if (this.cev.getValue().booleanValue()) {
            ...
            if (this.b_cev != null && this.getBlock(this.b_cev).getBlock() == Blocks.AIR) {
                ...
                if (crystal == null && (blockPos = new BlockPos(Blocker.mc.player.posX, (double)this.b_cev.getY(), Blocker.mc.player.posZ)).getDistance(this.b_cev.getX(), this.b_cev.getY(), this.b_cev.getZ()) < 1.0) {
                    int oldslot = Blocker.mc.player.inventory.currentItem;
                    Blocker.mc.player.inventory.currentItem = ob;
                    Blocker.mc.playerController.updateController();
