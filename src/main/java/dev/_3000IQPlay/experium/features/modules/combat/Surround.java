...
public class Surround extends Module
{
    ...
    public void onEnable() {
        ...
        if (this.center.getValue()) {
            Experium.positionManager.setPositionPacket(this.startPos.getX() + 0.5, this.startPos.getY(), this.startPos.getZ() + 0.5, true, true, true);
        }
        this.retries.clear();
    ...
    private Vec3d areClose(final Vec3d[] vec3ds) {
        ...
        for (final Vec3d vec3d : vec3ds) {
            for (final Vec3d pos : OyVeyentityUtil.getUnsafeBlockArray((Entity)Surround.mc.player, 0, true)) {
        ...
        for (final Vec3d vec3d : vec3ds) {
            ...
            switch (BlockUtil.isPositionPlaceable(position, false)) {
                case 1: {
                    ...
                    if (Experium.speedManager.getSpeedKpH() != 0.0 || isExtending) {
                        break;
                    }
                    if (this.extenders >= 1) {
                        break;
