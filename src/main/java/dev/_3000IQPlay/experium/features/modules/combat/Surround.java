...
extends Module {
    ...
    private void doFeetPlace() {
        ...
        if (!OyVeyentityUtil.isSafe((Entity)Surround.mc.player, 0, true)) {
            ...
            this.isSafe = 3;
            if (Util.mc.world.getBlockState(EntityUtil.getRoundedBlockPos((Entity)Util.mc.player)).getBlock().equals((Object)Blocks.ENDER_CHEST) && Util.mc.player.posY != (double)EntityUtil.getRoundedBlockPos((Entity)Util.mc.player).getY()) {
                this.placeBlocks(Surround.mc.player.getPositionVector(), OyVeyentityUtil.getUnsafeBlockArray((Entity)Surround.mc.player, 1, false), false, false, true);
            } else {
                this.isSafe = 4;
            }
    ...
    private Vec3d areClose(Vec3d[] vec3ds) {
        ...
        for (Vec3d vec3d : vec3ds) {
            for (Vec3d pos : OyVeyentityUtil.getUnsafeBlockArray((Entity)Surround.mc.player, 0, true)) {
