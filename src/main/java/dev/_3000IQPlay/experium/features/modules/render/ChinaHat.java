        ...
        if (ChinaHat.mc.gameSettings.thirdPersonView != 0 || this.firstP.getValue().booleanValue()) {
            for (int i = 0; i < 400; ++i) {
                ...
                if (ChinaHat.mc.player.isElytraFlying()) {
                    ChinaHatUtil.drawHat((Entity)ChinaHat.mc.player, 0.009 + (double)i * 0.0014, render3DEvent.getPartialTicks(), this.points.getValue(), 2.0f, 1.1f - (float)i * 7.85E-4f - (ChinaHat.mc.player.isSneaking() ? 0.03f : 0.03f), (int)f);
                    continue;
                }
                ChinaHatUtil.drawHat((Entity)ChinaHat.mc.player, 0.009 + (double)i * 0.0014, render3DEvent.getPartialTicks(), this.points.getValue(), 2.0f, 2.2f - (float)i * 7.85E-4f - (ChinaHat.mc.player.isSneaking() ? 0.03f : 0.03f), (int)f);
            }
        }
    }
}
