...
extends Module {
    ...
    private void doHoleFill() {
        ...
        if (this.midSafeHoles.getValue().booleanValue()) {
            object2 = Experium.holeManager.getMidSafety();
            object = object2;
            synchronized (object) {
                targets = new ArrayList<BlockPos>(Experium.holeManager.getMidSafety());
            }
        }
        object2 = Experium.holeManager.getHoles();
        object = object2;
        synchronized (object) {
        ...
        for (BlockPos position : targets) {
            ...
            if ((placeability = BlockUtil.isPositionPlaceable(position, this.raytrace.getValue())) == 1 && (this.currentMode == Mode.WEBS || this.switchMode.getValue() == InventoryUtil.Switch.SILENT && (this.currentMode == Mode.WEBS || this.retries.get((Object)position) == null || this.retries.get((Object)position) < 4))) {
