...
public class ExperiumCSGO extends Module
{
    public ExperiumCSGO() {
        super("ExperiumCSGO", "Hot csgo watermark", Category.CLIENT, true, false, false);
    }
    ...
    public void onRender(final RenderGameOverlayEvent.Post event) {
        ...
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
            final String ping = this.getPing((EntityPlayer)ExperiumCSGO.mc.player) + "ms";
            final String fpsText = Minecraft.debugFPS + "fps ";
            final String server = Minecraft.getMinecraft().isSingleplayer() ? "singleplayer".toLowerCase() : ExperiumCSGO.mc.getCurrentServerData().serverIP.toLowerCase();
            final String text = "Experium | " + server + " | " + ping + " | " + fpsText;
            ...
            drawRect(posX, posY, posX + width + 2.0f, posY + height, new Color(30, 30, 30, 255).getRGB());
    ...
    private int getPing(final EntityPlayer player) {
        int ping = 0;
        try {
            ping = (int)MathUtil.clamp((float)Objects.requireNonNull(ExperiumCSGO.mc.getConnection()).getPlayerInfo(player.getUniqueID()).getResponseTime(), 1.0f, 300.0f);
        }
