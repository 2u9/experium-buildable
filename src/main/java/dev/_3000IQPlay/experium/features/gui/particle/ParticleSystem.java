    ...
    public void update() {
        for (int i = 0; i < 200; ++i) {
            ...
            if (this.scaledResolution != null) {
                ...
                final boolean bl;
                final boolean isOffScreenY = bl = (particle.getPos().y > this.scaledResolution.getScaledHeight() || particle.getPos().y < 0.0f);
        ...
        for (int i = 0; i < 200; ++i) {
            ...
            for (int j = 1; j < 200; ++j) {
                if (i != j) {
                    ...
                    if (diff < distance) {
                        final int lineAlpha;
                        if ((lineAlpha = (int)map(diff, distance, 0.0, 0.0, 127.0)) > 8) {
