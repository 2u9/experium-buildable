...
public class OyVeyentityUtil implements Util
{
    ...
    public static Vec3d getInterpolatedRenderPos(final Entity entity, final float partialTicks) {
        return getInterpolatedPos(entity, partialTicks).subtract(OyVeyentityUtil.mc.getRenderManager().renderPosX, OyVeyentityUtil.mc.getRenderManager().renderPosY, OyVeyentityUtil.mc.getRenderManager().renderPosZ);
    }
    ...
    public static List<Vec3d> getUnsafeBlocksFromVec3d(final Vec3d pos, final int height, final boolean floor) {
        ...
        for (final Vec3d vector : getOffsets(height, floor)) {
            ...
            final Block block = OyVeyentityUtil.mc.world.getBlockState(targetPos).getBlock();
    ...
    public static boolean isObbyHole(final BlockPos blockPos) {
        ...
        for (final BlockPos pos : array) {
            final IBlockState touchingState = OyVeyentityUtil.mc.world.getBlockState(pos);
    ...
    public static boolean isInWater(final Entity entity) {
        ...
        for (int x = MathHelper.floor(entity.posX); x < MathHelper.ceil(entity.posX); ++x) {
            for (int z = MathHelper.floor(entity.posZ); z < MathHelper.ceil(entity.posZ); ++z) {
                ...
                if (OyVeyentityUtil.mc.world.getBlockState(pos).getBlock() instanceof BlockLiquid) {
                    return true;
                }
            }
        }
    ...
    public static List<Vec3d> targets(final Vec3d vec3d, final boolean antiScaffold, final boolean antiStep, final boolean legs, final boolean platform, final boolean antiDrop, final boolean raytrace) {
        final ArrayList<Vec3d> placeTargets = new ArrayList<Vec3d>();
        if (antiDrop) {
            Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, OyVeyentityUtil.antiDropOffsetList));
        }
        if (platform) {
            Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, OyVeyentityUtil.platformOffsetList));
        }
        if (legs) {
            Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, OyVeyentityUtil.legOffsetList));
        }
        Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, OyVeyentityUtil.OffsetList));
        if (antiStep) {
            Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, OyVeyentityUtil.antiStepOffsetList));
        }
        else {
            ...
            if (vec3ds.size() == 4) {
                for (final Vec3d vector : vec3ds) {
                    ...
                    if (antiScaffold) {
                        Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, OyVeyentityUtil.antiScaffoldOffsetList));
                    }
                    return placeTargets;
                }
            }
    ...
    public static boolean canEntityFeetBeSeen(final Entity entityIn) {
        return OyVeyentityUtil.mc.world.rayTraceBlocks(new Vec3d(OyVeyentityUtil.mc.player.posX, OyVeyentityUtil.mc.player.posX + OyVeyentityUtil.mc.player.getEyeHeight(), OyVeyentityUtil.mc.player.posZ), new Vec3d(entityIn.posX, entityIn.posY, entityIn.posZ), false, true, false) == null;
    }
    
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals((Object)OyVeyentityUtil.mc.player) || (entity instanceof EntityPlayer && Experium.friendManager.isFriend(entity.getName())) || OyVeyentityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static double getMaxSpeed() {
        ...
        if (OyVeyentityUtil.mc.player.isPotionActive((Potion)Objects.requireNonNull(Potion.getPotionById(1)))) {
            maxModifier *= 1.0 + 0.2 * (Objects.requireNonNull(OyVeyentityUtil.mc.player.getActivePotionEffect((Potion)Objects.requireNonNull(Potion.getPotionById(1)))).getAmplifier() + 1);
        }
        return maxModifier;
    }
    ...
    public static void moveEntityStrafe(final double speed, final Entity entity) {
        if (entity != null) {
            final MovementInput movementInput = OyVeyentityUtil.mc.player.movementInput;
    ...
    public static boolean isMoving() {
        return OyVeyentityUtil.mc.player.moveForward != 0.0 || OyVeyentityUtil.mc.player.moveStrafing != 0.0;
    }
    
    public static EntityPlayer getClosestEnemy(final double distance) {
        EntityPlayer closest = null;
        for (final EntityPlayer player : OyVeyentityUtil.mc.world.playerEntities) {
            ...
            if (closest == null) {
                ...
                if (OyVeyentityUtil.mc.player.getDistanceSq((Entity)player) >= OyVeyentityUtil.mc.player.getDistanceSq((Entity)closest)) {
                    continue;
                }
                closest = player;
            }
    ...
    public static boolean checkCollide() {
        return !OyVeyentityUtil.mc.player.isSneaking() && (OyVeyentityUtil.mc.player.getRidingEntity() == null || OyVeyentityUtil.mc.player.getRidingEntity().fallDistance < 3.0f) && OyVeyentityUtil.mc.player.fallDistance < 3.0f;
    }
    ...
    public static double[] forward(final double speed) {
        ...
        float yaw = OyVeyentityUtil.mc.player.prevRotationYaw + (OyVeyentityUtil.mc.player.rotationYaw - OyVeyentityUtil.mc.player.prevRotationYaw) * OyVeyentityUtil.mc.getRenderPartialTicks();
        if (forward != 0.0f) {
            if (side > 0.0f) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : OyVeyentityUtil.mc.world.playerEntities) {
            if (!player.isInvisible()) {
                if (player.getName().equals(OyVeyentityUtil.mc.player.getName())) {
                    continue;
                }
                ...
                healthSB.append(hp);
                final int distanceInt = (int)OyVeyentityUtil.mc.player.getDistance((Entity)player);
                ...
                output.put(healthSB.toString() + " " + (Experium.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB.toString() + " \u00c2§f0", (int)OyVeyentityUtil.mc.player.getDistance((Entity)player));
